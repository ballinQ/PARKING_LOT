...
Save all outputs in the project root unless the command below specifies another path.
...
4. `SmartParkingReminder/TestResults/xcodebuild_<timestamp>.log`
   - Full raw `xcodebuild test` output.
5. `SmartParkingReminder/TestResults/Phase1Tests_<timestamp>.xcresult`
   - Xcode result bundle from the automated run.
6. Manual evidence, when applicable:
   - screenshots, screen recordings, or short notes under `SmartParkingReminder/TestResults/manual_<timestamp>/`.
...
```bash
mkdir -p SmartParkingReminder/TestResults
TS=$(date +%Y%m%d_%H%M%S)
xcodebuild test \
  -project "SmartParkingReminder/SmartParkingReminder.xcodeproj" \
  -scheme SmartParkingReminder \
  -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.2' \
  -resultBundlePath "SmartParkingReminder/TestResults/Phase1Tests_${TS}.xcresult" \
  2>&1 | tee "SmartParkingReminder/TestResults/xcodebuild_${TS}.log"
```
...
- If address search does not relocate the map or filter nearby history, inspect `HistoryMapViewModel.searchAddress()`.
...
Create/update `PHASE1_TEST_REPORT_DATA.json` after the automated and manual pass. Use this structure:
...
```json
{
  ...
  "phase": "Phase 1",
  "xcresultPath": "SmartParkingReminder/TestResults/Phase1Tests_20260424_091500.xcresult",
  "xcodebuildLogPath": "SmartParkingReminder/TestResults/xcodebuild_20260424_091500.log",
  "testCases": [
    {
      "id": "TC-01",
      "feature": "F1/F2/F3",
...
```bash
node scripts/generate_phase1_report.mjs .
```
